...
prop_hdSetWork ::
  ...
  Time.TimeOfDay ->
  Property
prop_hdSetWork runDB day tid project office arrivedWithSeconds leftWithSeconds = Q.monadic (ioProperty . runDB) $ do
  let arrived = toMinute arrivedWithSeconds
      left = toMinute leftWithSeconds
  exceptionRaised <-
    Q.run
      $ catch
        ( do
...
-- | Normalize a 'TimeOfDay' to the minute. The application only handles
-- hour/minute precision and the sqlite backend does not persist the sub-second
-- part of a 'TimeOfDay'. Times generated by QuickCheck are therefore
-- normalized before being stored and compared.
toMinute :: Time.TimeOfDay -> Time.TimeOfDay
toMinute tod = tod {Time.todSec = 0}
...
prop_hdSetArrived runDB day tid project todWithSeconds = Q.monadic (ioProperty . runDB) $ do
  let tod = toMinute todWithSeconds
...
prop_hdSetLeft runDB day tid project todWithSeconds = Q.monadic (ioProperty . runDB) $ do
  let tod = toMinute todWithSeconds
  -- Initialize the two hdws
  (mbWorked, mbOWorked) <- Q.run $ do
    projAdd project
...
prop_hdSetArrivedAndLeft ::
  ...
  Time.TimeOfDay ->
  Property
prop_hdSetArrivedAndLeft runDB day tid project arrivedWithSeconds leftWithSeconds = Q.monadic (ioProperty . runDB) $ do
  let arrived = toMinute arrivedWithSeconds
      left = toMinute leftWithSeconds
  ...
  describe "Test the project API" $ do
    context "When the DB is empty" $ do
      ...
      it "tests if a project does not exists"
        $ runDB (projExists project1)
        `shouldReturn` False
      it "tests if we can remove a project"
        $ runDB (projRm project1)
...
itemsNoWorkedEntry runDB = do
  it "tests setting arrived time"
    ...
    `shouldThrow` hdNotFoundException
  ...
  it "tests setting the project"
    $ runDB (hdSetProject day1 tid1 project1)
    `shouldThrow` hdNotFoundException
    ...
    context "One user in the DB"
      ...
      $ do
        ...
        it "check a wrong password"
          $ runDB (userCheck user1 password2)
          `shouldReturn` False

...
data DefaultHours = DefaultHours
    { _defaultHoursArrived :: !Time.TimeOfDay,
...
getConfig = do
    ...
    pure $
        config & dbConfig %~ (connectionString %~ (`fromMaybe` mbDatabaseUrl))
